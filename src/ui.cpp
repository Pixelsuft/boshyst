...
#undef min
#undef max
template <typename T> inline T mclamp(T v, T minv, T maxv) {
...
static void post_draw() {
    if (conf.draw_cursor) {
        ...
        if (x >= 0 && y >= 0) {
            ...
            draw_list->AddCircleFilled(ImVec2((float)x, (float)y), 3.f, IM_COL32(255, 0, 0, 255),
                                       8);
        }
    }
}
...
static void draw_basic_text() {
    ...
    if (0) {
        ...
        for (int i = 0; i < pState.objectCount; i++) {
            ...
            int cur_x = obj->xPos;
            int cur_y = obj->yPos;
            if (cur_x <= pState.currentViewportX || cur_x >= (pState.currentViewportX + 640) ||
                cur_y <= pState.currentViewportY || cur_y >= (pState.currentViewportY + 480))
                continue;
            char buf[16];
            _itoa((int)i, buf, 10);
            auto pos = ImVec2((float)(cur_x - pState.currentViewportX),
                              (float)(cur_y - pState.currentViewportY));
            auto pos2 = ImVec2((float)(cur_x - pState.currentViewportX + 20),
                               (float)(cur_y - pState.currentViewportY + 10));
