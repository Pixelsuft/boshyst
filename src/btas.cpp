...
static void import_replay(const std::string& path) {
    ...
    string line;
    if (!f.read_line(line) || !startsWith(line, "-4,brep,2")) {
    ...
    while (f.read_line(line)) {
        ...
        if (end == p || *end != ',') {
            last_msg = "Invalid Frame";
            return;
        }
        p = end + 1;
        ev.idx = (uint8_t)std::strtol(p, &end, 10);
        if (end == p || *end != ',') {
            last_msg = "Invalid IDX";
            return;
        }
        p = end + 1;
...
static void export_replay(const std::string& path) {
    ...
    for (auto it = st.ev.begin(); it != st.ev.end(); it++) {
        ...
        if (!export_hash && (ev.idx == 3 || ev.idx == 4))
            continue;
        if (ev.idx == 5 || ev.idx == 6) {
            ASS(f.write_line(to_str(ev.frame) + ',' + to_str((int)ev.idx) + ',' +
                             to_str(ev.click.x) + ',' + to_str(ev.click.y)));
        } else {
            ASS(f.write_line(to_str(ev.frame) + ',' + to_str((int)ev.idx) + ',' +
                             to_str(ev.click.x)));
        }
...
void btas::read_setting(const string& line, const string& line_orig) {
    ...
    if (starts_with(line, "btas=map,")) {
        ...
        while (!need_replay_load.empty() &&
               (isspace(need_replay_load[0]) || need_replay_load[0] == ','))
            need_replay_load = need_replay_load.substr(1);
    } else {
...
#define WPM(addr, data, size)                                                                      \
    do {                                                                                           \
        ASS(WriteProcessMemory(hproc, (LPVOID)(addr), data, size, &bW) != 0 && bW == size);        \
    } while (0)
...
template <typename T> static void write_bin(bfs::File& f, T data) {
    ASS(f.write(&data, sizeof(T)));
}

template <typename T> static void load_bin(bfs::File& f, std::vector<T>& data) {
    size_t size;
...
static void exec_event(BTasEvent& ev) {
    ...
    case 3: {
        ...
        if (comp_val != ev.hash.val) {
            last_msg =
                string("Hash check (POS) failed on frame ~") + to_str(st.frame) + ", resetting";
    ...
    case 4: {
        ...
        if (comp_val != ev.hash.val) {
            ...
            last_msg =
                string("Hash check (RNG) failed on frame ~") + to_str(st.frame) + ", resetting";
            ev.hash.val = comp_val;
        }
        break;
    }
    ...
    case 6: {
        ...
        std::sort(st.rng_buf.begin(), st.rng_buf.end(),
...
void btas::on_key(int k, bool pressed) {
    ...
    while (it != binds.end()) {
        ...
        if ((pressed && ((bind.mod != current_mod) || b_loading_saving_state)) ||
            (!pressed && !bind.down)) {
            it++;
            continue;
        }
